    ...
    protected void initialize() {
    	...
    	if(RobotMap.leftStick.getRawAxis(3) >= 0.5){
    		...
    		if(RobotMap.rightStick.getRawAxis(3) >= 0.5){
    			...
    			while(Robot.ultrasonic6.getValue() > 1){//replace 1 with the distance
    				double centerX;
    				synchronized (Robot.imgLock) {
    					centerX = Robot.centerX;
    				}
    				double turn = centerX - (320 / 2);
    				Robot.myRobot.arcadeDrive(-0.6, turn * 0.005);
    			}
    			RobotMap.flywheelVictor.set(-1);
    			Timer.delay(5);
